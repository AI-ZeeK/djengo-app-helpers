...
export type PhoneInputParts = { dialCode: string; phoneNumber: string };
...
function parseHexColorRgb(input: string | undefined): {
  r: number;
  g: number;
  b: number;
} | null {
...
function rgbToHex6(rgb: { r: number; g: number; b: number }): string {
  return (
    "#" +
    [rgb.r, rgb.g, rgb.b]
      .map((x) =>
...
export class AppHelper {
  ...
  private static relativeLuminanceChannel(c: number): number {
    const v = c / 255;
    return v <= 0.03928 ? v / 12.92 : Math.pow((v + 0.055) / 1.055, 2.4);
  }

  /** WCAG 2.x relative luminance (0–1). */
  static relativeLuminance(rgb: { r: number; g: number; b: number }): number {
   ...
   */
  static contrastingForegroundForBackground(
    ...
    if (fromHex) {
      ...
      const crDark = AppHelper.wcagContrastRatio(
        bgL,
        AppHelper.relativeLuminance(darkFg),
      );
      ...
      let bestCrLight = AppHelper.wcagContrastRatio(
        bgL,
        AppHelper.relativeLuminance(bestLight),
      );
      for (let i = 1; i < lightCandidates.length; i++) {
        const c = lightCandidates[i]!;
        const cr = AppHelper.wcagContrastRatio(
          bgL,
          AppHelper.relativeLuminance(c),
        );
        if (cr > bestCrLight) {
          bestCrLight = cr;
          bestLight = c;
   ...
   */
  static roleLevelUiFromAccent(
    accentCss: string | undefined,
    isDark: boolean,
  ): RoleLevelUiChrome {
    ...
    const border = mixRgb(
      accent,
      isDark ? { r: 255, g: 255, b: 255 } : { r: 15, g: 23, b: 42 },
      isDark ? 0.34 : 0.2,
    );
    const ring = `rgba(${accent.r},${accent.g},${accent.b},${isDark ? 0.34 : 0.25})`;

    const badgeBg = mixRgb(
      accent,
      isDark ? { r: 8, g: 12, b: 22 } : { r: 255, g: 255, b: 255 },
      isDark ? 0.42 : 0.14,
    );
  ...
  static clampDialCodeInput(value: string): string {
    return AppHelper.phoneDigits(value).slice(
      0,
      AppHelper.DIAL_CODE_MAX_LENGTH,
    );
  }
  ...
  static shouldAdvanceFromDialCode(digits: string): boolean {
    ...
    if (code.length >= AppHelper.DIAL_CODE_MAX_LENGTH) return true;
  ...
  static collectCallingCodeSplits(
    rawDigits: string,
    dialHint = "",
  ): SplitCandidate[] {
    ...
    for (const len of [3, 2, 1] as const) {
      if (digits.length <= len + 5) continue;
      const dialCode = digits.slice(0, len);
      if (!AppHelper.CALLING_CODES.has(dialCode)) continue;

      const national = AppHelper.normalizeNationalNumber(
        digits.slice(len),
        dialCode,
      );
      if (!AppHelper.isPlausibleNationalNumber(national)) continue;

      candidates.push({
        dialCode,
        nationalDigits: national,
        score: AppHelper.scorePhoneSplit(
          dialCode,
          national,
          digits.length,
          dialHint,
  ...
  static pickBestCallingCodeSplit(
    rawDigits: string,
    dialHint = "",
   ...
   */
  static splitLeadingCallingCode(
    rawDigits: string,
    dialHint = "",
  ): { dialCode: string; nationalDigits: string } | null {
   ...
   */
  static tryAutoSplitPhoneInput(
    dialCode: string,
    phoneNumber: string,
  ): PhoneInputParts | null {
    const dial = AppHelper.phoneDigits(dialCode);
    const national = AppHelper.phoneDigits(phoneNumber);

    if (dial && national) {
      const normalizedNational = AppHelper.normalizeNationalNumber(
        national,
        dial,
      );
      if (
        AppHelper.CALLING_CODES.has(dial) &&
  ...
  static parsePhoneClipboardText(
    text: string,
    dialHint = "",
  ): PhoneInputParts | null {
  ...
  static normalizeNationalNumber(phoneNumber: string, dialCode = ""): string {
  ...
  static phonesMatch(
    ...
    if (!a || !b) return false;
    if (dialA && dialB && dialA !== dialB) return false;
    if (a === b) return true;
    const tail = 9;
  ...
  static parseStoredPhone(
    storedPhone: string | null | undefined,
    storedDialCode?: string | null,
    ...
    if (!digits) return { dialCode: dialHint, nationalNumber: "" };

    if (dialHint && digits.startsWith(dialHint)) {
      return {
        dialCode: dialHint,
        nationalNumber: AppHelper.normalizeNationalNumber(
          digits.slice(dialHint.length),
          "",
        ),
      };
    }
